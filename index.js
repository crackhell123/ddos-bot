...
const token = '7086383600:AAGH0aMNnGMdCir663uDS86yP_KY5kTfxl8';
...
bot.on('message', (msg) => {
  ...
  if (command.startsWith('/crackhell')) {
    ...
    if (args.length === 5 && url && time && thread && rate) {
      ...
      exec(`node crackhell.js ${url} ${time} ${thread} ${rate}`, (error, stdout, stderr) => {
        if (error) {
      ...
      bot.sendMessage(chatId, 'Format pesan tidak benar. Gunakan format: /crackhell [url] [time] [thread] [rate]');
    }
  }
});
